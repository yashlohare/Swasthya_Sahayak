...
# Ensure UTF-8 output on Windows (mostly for local testing, Vercel is Linux)
...
vector_db = {
    ...
    "seizure": {
        ...
        "protocols": {
            ...
            "Tamil": "1. கடினமான లేదా కూర్மையான பொருட்களை అப்புறப்படுத்துங்கள். 2. அவர்களை அழுத்திப் பிடிக்காதீர்கள் அல்லது வாயில் எதையும் வைக்காதீர்கள். 3. வலிப்பு நின்ற பிறகு அவர்களை ఒకక్కளித்துப் పடுக்க வைக்கவும்.",
    ...
    "bleeding": {
        ...
        "protocols": {
            ...
            "Hindi": "1. साफ कपड़े से घाవ पर सीधा दबाव डालें। 2. यदि कपड़ा भीग जाए, तो उसे हटाएं नहीं; ऊपर से और कपड़ा रखें। 3. यदि खून ज्यादा बह रहा हो, तो 108 पर कॉल करें।",
...
# --- PDF KNOWLEDGE FALLBACK (Requires knowledge_base.json in root or same dir) ---
...
def ask_medllama(query: str, context: str, target_lang: str = "English", force_format: bool = False):
    # Vercel Serverless Functions usually don't have Ollama running locally.
    # We return the context as is for English, or the original context for other languages.
    return context
...
def triage_engine(query: QueryContext):
    ...
    if "protocols" in context_data and lang_key in context_data["protocols"]:
        ...
        if "snippets" in context_data:
            llm_output = " ".join(context_data["snippets"])
        else:
            llm_output = context_data["protocols"].get("English", "1. Stay calm. 2. Call emergency services.")
